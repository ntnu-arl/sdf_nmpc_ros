...
class RosWrapper:
    def __init__(self, cfg, tol=2.5e-2, nb_points=10000):
        ...
        img_to_points = Imgs2Points(False, self.cfg.sensor.is_spherical, self.cfg.sensor.dmax, self.cfg.sensor.hfov, self.cfg.sensor.vfov, remove_d0=True, remove_dmax=True, downsamp=2, device=self.cfg.nn.vae_device)
    ...
    def pc_to_msg(self, pc, norm=0., val=0.):
    ...
    def cb_latent(self, msg):
        with torch.no_grad():
            latent = torch.tensor(msg.data, dtype=torch.float32, device=self.cfg.nn.vae_device).reshape(1,-1)
            points_sdf = self.sdf(torch.hstack([self.points, latent.repeat(self.nb_points, 1)])).flatten()

            ## get lvlset
            idx = (points_sdf - self.cfg.mpc.bound_margin).abs() < self.tol
            lvlset = self.points[idx]
            norms = torch.norm(lvlset, dim=1)

            ## publish
            msg = self.pc_to_msg(lvlset.cpu().numpy(), norm=norms.cpu().numpy())
